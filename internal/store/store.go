...
func (s *Store) init(ctx context.Context) error {
	...
	ddl := []string{
		...
		`CREATE INDEX IF NOT EXISTS idx_sessions_project_root_recent ON sessions(project_id, started_at DESC, created_at DESC, id DESC) WHERE parent_session_id IS NULL OR parent_session_id = ''`,
		`CREATE INDEX IF NOT EXISTS idx_sessions_root_recent ON sessions(started_at DESC, created_at DESC, id DESC) WHERE parent_session_id IS NULL OR parent_session_id = ''`,
		`CREATE INDEX IF NOT EXISTS idx_sessions_active_activity ON sessions(COALESCE(last_activity, started_at)) WHERE status = 'active'`,
		`CREATE INDEX IF NOT EXISTS idx_sessions_active_parent ON sessions(parent_session_id) WHERE status = 'active'`,
		...
		`CREATE INDEX IF NOT EXISTS idx_events_subagent_stop_agent ON events(agent_id) WHERE subtype = 'SubagentStop'`,
